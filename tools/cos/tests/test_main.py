...
from cos_cli.config import DEFAULTS, ResolvedConfig, resolve_effective_config  # noqa: E402
...
def _resolved_config_stub(tmp_path: Path) -> ResolvedConfig:
    values = {key: str(value) for key, value in DEFAULTS.items()}
    sources = {key: "default" for key in values.keys()}
    return ResolvedConfig(
        values=values,
        sources=sources,
        global_config_path=tmp_path / "global.toml",
        local_config_path=tmp_path / "local.toml",
...
def test_parser_without_arguments_shows_help_and_exits_zero():
    runner = CliRunner()
...
def test_chat_command_defaults_to_regular_permission_flow(monkeypatch, tmp_path):
    ...
    resolved = _resolved_config_stub(tmp_path)

    monkeypatch.setattr("cos_cli.parser._resolve_runtime_config_or_exit", lambda **kwargs: resolved)
    monkeypatch.setattr("cos_cli.parser._resolved_values_with_backend_defaults", lambda cfg: dict(cfg.values))

    def fake_run_codex(**kwargs):
        calls.append(kwargs)
        return 0

    monkeypatch.setattr("cos_cli.parser._run_codex", fake_run_codex)

    runner = CliRunner()
    result = runner.invoke(app, ["chat"])
    assert result.exit_code == 0
    assert len(calls) == 1
    assert calls[0]["command"] == "chat"
    assert calls[0]["dangerous"] is False


def test_global_yolo_flag_runs_chat_with_dangerous_mode(monkeypatch, tmp_path):
    calls: list[dict[str, object]] = []
    resolved = _resolved_config_stub(tmp_path)
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["--yolo"])
    assert result.exit_code == 0
    assert len(calls) == 1
    assert calls[0]["command"] == "chat"
    assert calls[0]["dangerous"] is True


def test_global_dangerous_alias_flag_runs_chat_with_dangerous_mode(monkeypatch, tmp_path):
    calls: list[dict[str, object]] = []
    resolved = _resolved_config_stub(tmp_path)
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["--dangerously-bypass-approvals-and-sandbox"])
    assert result.exit_code == 0
    assert len(calls) == 1
    assert calls[0]["command"] == "chat"
    assert calls[0]["dangerous"] is True
...
def test_global_yolo_flag_applies_to_chat_subcommand(monkeypatch, tmp_path):
    calls: list[dict[str, object]] = []
    resolved = _resolved_config_stub(tmp_path)
    ...
    runner = CliRunner()
    result = runner.invoke(app, ["--yolo", "chat", "hello"])
    assert result.exit_code == 0
    assert len(calls) == 1
    assert calls[0]["command"] == "chat"
