...
def _require_workspace_skill(
    *,
    catalog_items: list[dict],
    skill_key: str,
) -> dict:
    item = next((entry for entry in catalog_items if entry.get("skill_key") == skill_key), None)
    if item is None:
        pytest.skip(f"workspace skill '{skill_key}' is not seeded in this bootstrap profile")
    return item
...
def test_apply_team_mode_skill_ensures_agent_users_and_project_roles(tmp_path: Path):
    ...
    catalog = client.get(f"/api/workspace-skills?workspace_id={workspace_id}")
    assert catalog.status_code == 200
    team_mode_skill = _require_workspace_skill(catalog_items=catalog.json()["items"], skill_key="team_mode")
