...
def test_create_and_patch_note(tmp_path: Path):
    ...
    created = client.post("/api/notes", json={"title": "Plan: Notes MVP", "workspace_id": ws_id, "body": "# Hello\n\n```py\nprint('x')\n```"})
...
def test_list_notes_search_and_filters(tmp_path: Path):
    ...
    client.post("/api/notes", json={"title": "Alpha", "workspace_id": ws_id, "body": "first"})
    client.post("/api/notes", json={"title": "Beta", "workspace_id": ws_id, "body": "second", "pinned": True})

    res = client.get(f"/api/notes?workspace_id={ws_id}&q=Beta")
    assert res.status_code == 200
    assert any(n["title"] == "Beta" for n in res.json()["items"])

    pinned_only = client.get(f"/api/notes?workspace_id={ws_id}&pinned=true")
...
def test_archive_and_restore_note(tmp_path: Path):
    ...
    note = client.post("/api/notes", json={"title": "Archive me", "workspace_id": ws_id}).json()
    archived = client.post(f"/api/notes/{note['id']}/archive")
    assert archived.status_code == 200
    assert archived.json()["ok"] is True

    active_list = client.get(f"/api/notes?workspace_id={ws_id}").json()["items"]
    assert all(n["id"] != note["id"] for n in active_list)

    archived_list = client.get(f"/api/notes?workspace_id={ws_id}&archived=true").json()["items"]
...
def test_command_id_idempotency_for_create_note(tmp_path: Path):
    ...
    first = client.post(
        "/api/notes",
        json={"title": "Idempotent note", "workspace_id": ws_id},
    ...
    notes = client.get(f"/api/notes?workspace_id={ws_id}&q=Idempotent").json()["items"]
    assert len(notes) == 1
