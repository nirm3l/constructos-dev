...
const START_PROMPT_GUIDANCE_TEMPLATE = [
  ...
  '- For project creation, use an interactive flow only when required inputs are missing or ambiguous.',
  '- Ask one clarifying question at a time, and only for fields that are actually missing.',
  '- Infer setup strategy from user intent: default to manual when no template is requested.',
  '- Do not ask \'template vs manual\' when the request clearly describes a custom/manual setup.',
  '- Treat a direct create command in the current turn (for example \'create a new project ...\') as creation confirmation.',
  '- Ask for explicit confirmation only when create intent is uncertain.',
  '- Minimum manual flow: collect missing required fields -> create_project.',
  '- Template flow: list_project_templates -> get_project_template -> collect template parameters -> preview_project_from_template -> create_project_from_template.',
